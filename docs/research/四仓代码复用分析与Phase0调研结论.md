...
| 阶段 | 文档 | 说明 |
| --- | --- | --- |
| Phase 1 | `docs/plans/01-MVP落地计划.md` | 文本导购主链路，跑通 FastAPI、SQLite、DeepSeek、bge-m3、Chroma、LangGraph、SSE、商品卡片 |
| Phase 2 | `docs/plans/02-多模态检索计划.md` | 接入 Chinese-CLIP，完成图片上传、商品图片向量库、看图找类似款 |
| Phase 3 | `docs/plans/03-Android展示端计划.md` | Kotlin + Jetpack Compose 正式展示端，支持流式聊天、商品卡片、图片上传、反馈 |
| Phase 4 | `docs/plans/04-质量评测与反馈闭环计划.md` | 意图、检索、推荐、图片召回、用户反馈的评测体系 |
| Phase 5 | `docs/plans/05-工程化与部署计划.md` | Docker Compose、配置管理、日志、索引重建、测试和部署文档 |

因此，本文件只保留 Phase 0 的调研和复用判断，不再承担 MVP 执行计划职责。MVP 的详细任务、验收标准和执行顺序以 `docs/plans/01-MVP落地计划.md` 为准。
...
下一步进入 Phase 1，执行 `docs/plans/01-MVP落地计划.md`。
