...
这些目录不属于 GitHub 仓库内容，只是本机运行或调研时可能出现：

- `models/`：本地 bge-m3、Chinese-CLIP 等模型权重缓存。删除后不影响源码，但需要重新下载模型才能跑真实本地 embedding。
- `repos/`：早期调研四个参考仓库时使用的本地副本。正式项目不依赖它，确认不再查看参考代码后可以直接删除。
